...
contract WYStake {
    address public stakingToken = address(0); // 质押代币只能为ETH
    ...
    constructor(address _rewardToken, uint _minStakeAmount, uint _duration) {
        owner = msg.sender;
        rewardToken = IERC20(address(_rewardToken)); // 奖励代币为WY
        minStakeAmount = _minStakeAmount; // 设置最小质押金额
        duration = _duration; // 设置质押持续时间
    }
    ...
    function stake() external payable updateReward(msg.sender) {
        require(msg.value >= minStakeAmount, "Amount is less than min stake amount");
        balanceOf[msg.sender] += msg.value; // 记录用户质押的金额
        totalSupply += msg.value; // 更新质押代币总量
    }
    ...
    function withdraw(uint _amount) external updateReward(msg.sender) {
        ...
        payable(msg.sender).transfer(_amount); // 将代币转回给用户
    }
